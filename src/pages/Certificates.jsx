    ...
    <main className="min-h-screen bg-base text-[#e8e6e0] px-6 md:px-8 py-10">
      <section className="max-w-[1100px] mx-auto">
        <header className="mb-8">
          <p className="text-[0.72rem] uppercase tracking-[0.2em] text-[#f59e0b] mb-2">Achievements</p>
          <h1 className="font-heading text-[2rem] md:text-[2.3rem] text-text-primary mb-2">Your Certificates</h1>
        ...
        {completedCourseList.length === 0 ? (
          <section className="rounded-xl border border-[rgba(255,255,255,0.07)] bg-surface p-10 text-center">
          ...
          <ul className="grid gap-5 list-none p-0 m-0" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))" }}>
            {completedCourseList.map((course) => (
              <li key={course.id}>
                <article className="h-full rounded-xl border border-[rgba(255,255,255,0.08)] bg-surface p-5 flex flex-col">
                  <p className="text-[0.72rem] tracking-[0.18em] uppercase text-[#f59e0b] mb-2">Certificate Ready</p>
