...
export default function LessonEditor({ lesson, lessonIndex, onUpdate, onRemove, errors = {} }) {
  ...
  return (
    <div
      className="rounded-lg p-4 flex flex-col gap-3 bg-[#111114] border border-[rgba(255,255,255,0.05)]"
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            type="button"
            onClick={onRemove}
            className="text-[0.78rem] font-medium cursor-pointer bg-transparent border-none rounded focus-visible:ring-2 focus-visible:ring-amber-600/50 focus-visible:outline-none text-[#ef4444]"
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <FormField label="Duration *" htmlFor={`les-dur-${lesson.id}`} error={errors.duration}>
          <input
            ...
            className={`${INPUT_CLASS} ${buildInputBorder(errors.duration)}`}
          />
        </FormField>
      </div>
      ...
      <FormField label="Content Preview" htmlFor={`les-preview-${lesson.id}`} hint="Brief description of lesson content">
        <textarea
          ...
          rows={2}
          className={`${INPUT_CLASS} resize-y min-h-[60px] ${buildInputBorder(false)}`}
        />
      </FormField>
      ...
      <FormField label="Video URL" htmlFor={`les-video-${lesson.id}`} hint="Optional" error={errors.videoUrl}>
        <input
          ...
          className={`${INPUT_CLASS} ${buildInputBorder(errors.videoUrl)}`}
        />
      </FormField>
          ...
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <FormField label="Quiz Title" htmlFor={`quiz-title-${lesson.id}`}>
                <input id={`quiz-title-${lesson.id}`} value={quiz.title} onChange={(e) => updateQuiz({ title: e.target.value })} placeholder="Quick check" className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
              </FormField>
              <FormField label="Time Limit" htmlFor={`quiz-time-${lesson.id}`}>
                <input id={`quiz-time-${lesson.id}`} type="number" min="1" max="240" value={quiz.timeLimitMinutes} onChange={(e) => updateQuiz({ timeLimitMinutes: Number(e.target.value) })} className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
              </FormField>
              <FormField label="Pass %" htmlFor={`quiz-pass-${lesson.id}`}>
                <input id={`quiz-pass-${lesson.id}`} type="number" min="1" max="100" value={quiz.passingScore} onChange={(e) => updateQuiz({ passingScore: Number(e.target.value) })} className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
              </FormField>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-[1fr_160px] gap-3">
              <FormField label="Instructions" htmlFor={`quiz-instructions-${lesson.id}`}>
                <input id={`quiz-instructions-${lesson.id}`} value={quiz.instructions} onChange={(e) => updateQuiz({ instructions: e.target.value })} placeholder="Answer all questions before the timer ends." className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
              </FormField>
              <FormField label="Max Attempts" htmlFor={`quiz-attempts-${lesson.id}`}>
                <input id={`quiz-attempts-${lesson.id}`} type="number" min="1" max="20" value={quiz.maxAttempts} onChange={(e) => updateQuiz({ maxAttempts: Number(e.target.value) })} className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
              </FormField>
            </div>
            ...
            {quiz.questions.map((question, questionIndex) => (
              <div key={question.id || question._id || questionIndex} className="rounded-lg bg-[#0d0d10] border border-[rgba(255,255,255,0.05)] p-3 flex flex-col gap-3">
                ...
                <FormField label="Prompt" htmlFor={`quiz-prompt-${lesson.id}-${questionIndex}`}>
                  <input id={`quiz-prompt-${lesson.id}-${questionIndex}`} value={question.prompt || ""} onChange={(e) => updateQuestion(questionIndex, { prompt: e.target.value })} placeholder="What should the learner know?" className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
                </FormField>
                <div className="grid grid-cols-1 sm:grid-cols-[1fr_120px] gap-3">
                  <div className="flex flex-col gap-2">
                    {(question.options || ["", ""]).map((option, optionIndex) => (
                      <label key={optionIndex} className="grid grid-cols-[auto_1fr] items-center gap-2">
                        ...
                        <input value={option} onChange={(e) => updateOption(questionIndex, optionIndex, e.target.value)} placeholder={`Option ${optionIndex + 1}`} className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
                      </label>
                    ))}
                  ...
                  <FormField label="Points" htmlFor={`quiz-points-${lesson.id}-${questionIndex}`}>
                    <input id={`quiz-points-${lesson.id}-${questionIndex}`} type="number" min="1" max="20" value={question.points || 1} onChange={(e) => updateQuestion(questionIndex, { points: Number(e.target.value) })} className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
                  </FormField>
                </div>
                <FormField label="Explanation" htmlFor={`quiz-explain-${lesson.id}-${questionIndex}`} hint="Shown after submission">
                  <input id={`quiz-explain-${lesson.id}-${questionIndex}`} value={question.explanation || ""} onChange={(e) => updateQuestion(questionIndex, { explanation: e.target.value })} placeholder="Why is the correct answer right?" className={`${INPUT_CLASS} ${buildInputBorder(false)}`} />
                </FormField>
              </div>
            ))}
