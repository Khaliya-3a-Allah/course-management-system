...
3. In the Render dashboard **Environment** tab, add the one secret it cannot auto-set:
   - `MONGODB_URI` — your MongoDB Atlas connection string
4. `JWT_SECRET` is auto-generated by Render (`generateValue: true` in `render.yaml`).
...
**Auth required.** Generates a TOTP secret and returns a QR code URI.
...
**Auth required.** Confirms enrollment with a TOTP code from the authenticator app.
...
No auth required. Exchanges a `challengeToken` + TOTP code for a full session token.

**Request body**
```json
{ "challengeToken": "<short-lived jwt>", "token": "123456" }
```
